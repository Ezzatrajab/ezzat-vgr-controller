...
def load_rehab_intakter_from_pl(enhet_kst, manad_str):
    ...
    try:
        ...
        # Hitta rad för 3053 RG Prestationsersät Rehab
        rehab_row_actual = df_actual[df_actual['Unnamed: 3'].astype(str).str.contains('3053.*Rehab', case=False, na=False)]
        rehab_row_budget = df_budget[df_budget['Unnamed: 3'].astype(str).str.contains('3053.*Rehab', case=False, na=False)]

        if rehab_row_actual.empty or rehab_row_budget.empty:
            return {'actual': 0, 'budget': 0}

        # Hitta rätt kolumn baserat på månad
        # Header row (rad 0) innehåller månaderna
        # Kolumnerna är: 'CG Item', 'Unnamed: 1', 'Unnamed: 2', 'Unnamed: 3', 'Selected Period', 'Selected Period.1', etc
        # Selected Period kolumner börjar på index 4
        # Rad 0 innehåller: 'Year Month No', nan, nan, nan, 202601, 202602, 202603, ...

        # Hitta alla kolumner som börjar med 'Selected Period'
        ...
        actual_val = rehab_row_actual.iloc[0][col_idx]
        budget_val = rehab_row_budget.iloc[0][col_idx]
...
# Uppdatera data när appen laddar
...
def main():
    ...
    if page == "🏠 Översikt":
        ...
        if not is_rehab:
            ...
            with col4:
                st.markdown("#### 💪 Rehab Poäng (KPI)")
                actual = current_data.get('rehab_poang_kpi', 0)

                # Budget beräknas från 3053-intäkter
                ...
                if actual > 0 or budget > 0:
                    ...
                    st.metric("Poäng (från KPI)", f"{actual:,.0f}", f"{avv:+,.0f} ({avv_pct:+.1f}%)")
                    st.markdown(f"{traffic} Budget: {budget:,.0f}")
                else:
            ...
            with col1:
                st.markdown("#### 💰 Rehab Intäkter (P&L)")
            ...
            with col2:
                st.markdown("#### 💪 Rehab Poäng (KPI)")
                actual = current_data.get('rehab_poang_kpi', 0)
                # För budget använder vi samma beräkning som tidigare (från P&L budget)
                budget_intakter = current_data['intakter_3053']['budget']
                budget = calculate_rehab_poang(budget_intakter)
